...
class Chapter5ModelTests(TestCase):
    ...
    """
    def setUp(self):
        songs_py = Songs.objects.get_or_create(name='Song_1', name='Song_1.spot', name='Song_1.youtube', genre ="pop", coverArt = "test.png")
        user_1 = User.objects.get_or_create(username='testuser_1',
                                      first_name='Test',
                                      last_name='User',
                                      email='test@test.com')[0]
        user_1.set_password('testabc123')
        user_1.save()
        user_2 = User.objects.get_or_create(username='testuser_2',
                                      first_name='Test',
                                      last_name='User',
                                      email='test@test.com')[0]
        user_2.set_password('testabc123')
        user_2.save()
        userProfile_1_py = UserProfile.objects.get_or_create(user = user_1,
                                                            picture = None,
                                                            artist = True,
                                                            review = False,
                                                            artsIndusry = True)
        userProfile_2_py = UserProfile.objects.get_or_create(user = user_2,
                                                            picture = None,
                                                            artist = False,
                                                            review = True,
                                                            artsIndusry = False)
        userProfile_2_py.likedSong.add(songs_py)
        userProfile_2_py.save()                                                                                               

        blog_py =  Blogs.objects.get_or_create(title = "title",
                                                date = datetime(2015, 10, 09, 23, 55, 59, 342380),
                                                text = "text",
                                                postedBy =  userProfile_2_py)
        comment_py =  Comments.objects.get_or_create(content = "content",
                                                date = datetime(2016, 10, 09, 23, 55, 59, 342380),
                                                text = "text",
                                                postedBy =  userProfile_1_py)
